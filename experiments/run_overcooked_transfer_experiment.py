"""Overcooked cross-human transfer experiment with population training.

Phase 1: Train on humans with related preferences
Phase 2: Evaluate on new introduced human

Usage:
  python experiments/run_overcooked_transfer_experiment.py \
...
POPULATION_HUMAN_CONFIGS: list[tuple[str, str]] = [
    ("human_realistic", "RealisticCook"),
    ("human_robot_centric", "RobotCentric"),
    ("human_prep", "PrefsPrep"),
    ("human_delivery", "PrefsDelivery"),
    ("human_strong", "StrongCook"),
]

NEW_HUMAN_CONFIG: tuple[str, str] = ("human_new", "MildPrep")


def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Overcooked cross-human transfer")
    ...
    p.add_argument("--phase1-steps", type=int, default=2000,
                   help="Training steps per population human")
    p.add_argument("--phase2-steps", type=int, default=2000,
                   help="Training steps on new human")
...
def _run_training_phase(
    ...
    try:
        ...
        pref_gen._hbm.register_human(human_id)
    
        layout_name = env.unwrapped.scene_spec.layout_spec.name
        if hasattr(pref_gen._hbm, "register_layout"):
            pref_gen._hbm.register_layout(human_id, layout_name)
...
def main() -> None:
    ...
    if args.output_dir:
        ...
        now = datetime.now()
        out_dir = Path("logs") / "runs" / "oc_transfer_5h" / f"{args.approach}_seed{args.seed}"
    out_dir.mkdir(parents=True, exist_ok=True)
    model_dir = out_dir / "models"
    model_dir.mkdir(exist_ok=True)

    logging.info(f"Output directory: {out_dir}")
    logging.info(f"Approach: {args.approach}, Seed: {args.seed}, Layout: {args.layout}")
    ...
    # =====================================================================
    logging.info("=" * 60)
    logging.info("PHASE 1: Population training (5 humans)")
    logging.info("=" * 60)

    dummy_hidden = build_overcooked_hidden_spec(args.layout, POPULATION_HUMAN_CONFIGS[0][1])
    ...
    for human_label, config_name in POPULATION_HUMAN_CONFIGS:
        ...
        hidden_spec = build_overcooked_hidden_spec(args.layout, config_name)
    ...
    def _make_new_human_env(seed: int) -> Any:
        hidden = build_overcooked_hidden_spec(args.layout, new_human_config)
        return OvercookedEnv(layout_spec=layout_spec, hidden_spec=hidden, seed=seed)

    warm_train_env = _make_new_human_env(args.seed + 1000)
    cold_train_env = _make_new_human_env(args.seed + 1000)
    new_eval_env = _make_new_human_env(args.seed + 2000)
    ...
    try:
        ...
        pref_gen._human_id = new_human_label
        pref_gen._hbm.register_human(new_human_label)
        if hasattr(pref_gen._hbm, "register_layout"):
            pref_gen._hbm.register_layout(new_human_label, args.layout)
    except AttributeError:
        pass
    ...
    warm_eval.eval()
    try:
        pref_gen = warm_eval._csp_generator._pref_gen
        pref_gen._human_id = new_human_label
    except AttributeError:
        pass
    ...
    cold_eval.eval()
    try:
        pref_gen = cold_eval._csp_generator._pref_gen
        pref_gen._human_id = new_human_label
    except AttributeError:
        pass
    ...
    # Each approach trains on its own env
